...
So far, we have the contents of the post in the Markdown format, the top comments with its metadata. We need an LLM to summarize the post, summarize and group the comments but topic and include the citations. I decided to take the easy road for this step and all all the requirements into a single prompt.
...
Note: Disclaimer, I do work for Google but not on Gemini or Deepmind, opinions are my own.
...
The final summaries are written as a text file in the `library/` folder for now.
